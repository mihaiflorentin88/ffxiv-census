...
- `container`: Simple service locator that bootstraps configuration, logging, and optional infrastructure clients (Redis, StatsD, outbound HTTP, system helpers, UI assets). Generated code resolves these adapters through `ServiceContainer` accessors instead of constructing them inside handlers or domain services.
...
- `infrastructure`: Adapters that speak to the outside world (logging, Redis, metrics, etc.). Code here implements interfaces defined in `port/contract` to honour dependency inversion.
...
- Introduce message-queue or GRPC adapters under `infrastructure/`.
