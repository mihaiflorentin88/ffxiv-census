...
// Worker claims proxy-related jobs from the queue and dispatches them to registered
// handlers using a dynamic Dispatcher and Worker Pool pattern. It mirrors the
// census worker implementation.
type Worker struct {
...
// eventWorkerLoop is the claim-process loop for a single goroutine.
func (w *Worker) eventWorkerLoop(claimCtx context.Context, processCtx context.Context, primaryType string, allTypes []string, workerID int, isRetryWorker bool) error {
	...
	for {
		...
		// 1. Try to claim from primary dedicated queue using preferred mode.
		...
		// 2. If primary queue is empty, dynamically borrow work from other available queues.
		...
		// 4. All queues empty — wait for poll interval.
		select {
		case <-claimCtx.Done():
			return nil
...
// processJob dispatches a single job to its handler with panic isolation.
